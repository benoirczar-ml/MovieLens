...
Najlepszy wynik w obecnym setupie daje **final_ranker_ensemble** (4-way hybrid retrieval + ensemble rankera).

| Model | Recall@20 | NDCG@20 | MAP@20 | MRR@20 |
|---|---:|---:|---:|---:|
| baseline_retrieval_als | 0.1366 | 0.0515 | 0.0285 | 0.0285 |
| baseline_ranker | 0.1712 | 0.0708 | 0.0434 | 0.0434 |
| final_retrieval_two_tower_faiss_hybrid4 | 0.1350 | 0.0510 | 0.0283 | 0.0283 |
| final_ranker_two_tower_hybrid4 | 0.1778 | 0.0743 | 0.0459 | 0.0459 |
| final_ranker_ensemble (tuned fast) | **0.1791** | **0.0748** | **0.0462** | **0.0462** |

Wyniki top-line pochodzą z profilu `configs/fullrun_gpu_quality_goal_hybrid4_k220.yaml` (run z 9 marca 2026) i dostrojenia wag ensemble na zapisanych score'ach (`baseline=0.25`, `final=0.75`):
- [`outputs/fullrun_gpu_quality_goal_hybrid4_k220/tables/metrics_comparison.csv`](outputs/fullrun_gpu_quality_goal_hybrid4_k220/tables/metrics_comparison.csv)
- [`outputs/fullrun_gpu_quality_goal_hybrid4_k220/tables/ensemble_sweep_summary_k20_fast.csv`](outputs/fullrun_gpu_quality_goal_hybrid4_k220/tables/ensemble_sweep_summary_k20_fast.csv)
- [`outputs/fullrun_gpu_quality_goal_hybrid4_k220/tables/metrics_ensemble_tuned_fast_from_scores.csv`](outputs/fullrun_gpu_quality_goal_hybrid4_k220/tables/metrics_ensemble_tuned_fast_from_scores.csv)

Recruiter quick view (zamrozenie baseline): `docs/BASELINE_FREEZE_2026-03-09.md`

Milestone runtime (9 marca 2026): profile szybkościowe na pełnym runie i pełnych danych:
- `configs/fullrun_gpu_speed150.yaml`: **134.3s** (speed-first)
- `configs/fullrun_gpu_speed150_quality.yaml`: **145.2s** (lepsza jakość przy limicie <150s)

Milestone quality w budżecie ~150-200s:
- `configs/fullrun_gpu_quality_best.yaml` (alias `fullrun_gpu_quality_200_ep2_k70`): **198.4s**
- `final_ranker_two_tower @20`: Recall **0.1038**, NDCG **0.0430**, MAP **0.0263**

Milestone quality-max (full run, bez limitu czasu):
- `configs/fullrun_gpu_quality_goal_hybrid4_k220.yaml`: **2821.8s**
- `final_ranker_ensemble @20` (tuned fast): Recall **0.1791**, NDCG **0.0748**, MAP **0.0462**
- poprzedni quality-max `configs/fullrun_gpu_quality_goal_stable_cpufeat_k220_chunked.yaml`: Recall **0.1779**, NDCG **0.0741**, MAP **0.0457**, runtime **1923.8s**
- eksperyment `strict_gpufirst_k220` (więcej etapów na GPU) był słabszy jakościowo i wolniejszy: Recall **0.1757**, NDCG **0.0726**, MAP **0.0444**, runtime **2708.4s**
...
## Production Default (Quality)

Domyślny profil produkcyjny jest ustawiony na `strict` jakościowo (hybrid4 + tuned ensemble):
- config: `configs/production.yaml`
- launcher: `scripts/run_production.py`
...
- `speed150` (`configs/fullrun_gpu_speed150.yaml`): **134.3s**, metryki @20 (`final_ranker_two_tower`) Recall `0.0860`, NDCG `0.0363`
- `speed150_quality` (`configs/fullrun_gpu_speed150_quality.yaml`): **145.2s**, metryki @20 (`final_ranker_two_tower`) Recall `0.0902`, NDCG `0.0379`

Checkpointy:
- `checkpoints/2026-03-09_speed150_checkpoint/`
- `checkpoints/2026-03-09_speed150_quality_checkpoint/`
- `checkpoints/2026-03-09_quality200_k70_checkpoint/`

Porównanie profili speed:
- `outputs/tables/speed_profiles_compare_2026-03-09.csv`
...
python scripts/run_pipeline.py --config configs/fullrun_gpu_hybrid3_cosine_balanced.yaml
python scripts/run_pipeline.py --config configs/fullrun_gpu_hybrid3_cosine_gpuheavy.yaml
python scripts/run_pipeline.py --config configs/fullrun_gpu_speed150.yaml
python scripts/run_pipeline.py --config configs/fullrun_gpu_speed150_quality.yaml
python scripts/run_pipeline.py --config configs/fullrun_gpu_quality_best.yaml
python scripts/run_production.py
...
Przed przebudową pod runtime utworzony został checkpoint rollback:
- `checkpoints/2026-03-08_quality_strict_checkpoint/`
- zawiera config produkcyjny, log strict fullrun, metryki i `MANIFEST.sha256`.

Checkpoint po osiągnięciu celu runtime:
- `checkpoints/2026-03-09_speed150_checkpoint/`
- zawiera config speed150, log fullrun `134.3s`, metryki i `MANIFEST.sha256`.
- `checkpoints/2026-03-09_speed150_quality_checkpoint/`
- zawiera config speed150_quality, log fullrun `145.2s`, metryki i `MANIFEST.sha256`.
- `checkpoints/2026-03-09_quality200_k70_checkpoint/`
- zawiera config quality_best (`k70`, `epochs=2`), log fullrun `198.4s`, metryki i `MANIFEST.sha256`.
